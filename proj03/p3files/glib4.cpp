...
//Prints out the spawnpoint and spots if the player wins
...
//Converts the direction from a letter to a number
...
entity* smove(entity* ship, spawnpoint spawn, round data, int sspawn){
    ...
    for (int i = 1; i <= data.ship_num * (sspawn - 1); i++){
    ...
    for (int i = 1; i <= data.ship_num * (sspawn - 1); i++){//Draws the star after the spaces so that they always show up
...
entity* hmove(entity* hunter, entity player, spawnpoint spawn, round data, int hspawn){
    ...
    for (int i = 0; i < data.h_num * (spawn.count - 1); i++){//Draws the star after the spaces so that they always show up
...
int tracking(entity hunter, entity player){
    ...
    int cdir, rdir;
    //If statement that will set a direction based upon where the player is
    if (dc < 0){
        cdir = 3;
    }
    if (dc >= 0) {
...
//Prints out the ships at the start
for (int i = 1; i <= data.ship_num * (sspawn - 1); i++){
    drawChar('*', ships[i].loc.col, ships[i].loc.row);
}

//Prints out the Hunters
for (int i = 0; i < data.h_num * (spawn.count - 1); i++){
    drawChar('K', hunters[i].loc.col, hunters[i].loc.row);
}
...
//Determines if there was a collision with a ship or hunter
bool death(entity* ship, entity* hunter, entity player, spawnpoint spawn, round data, int sspawn){
    //Loop that checks all ship pos against the player's
    for (int i = 1; i <= data.ship_num * (sspawn - 1); i++){
        if ((equal(ship[i].loc, player.loc)) || ((equal(ship[i].prev, player.loc)) && equal(ship[i].loc, player.prev))){
            return true;
        }
    }
    //Loop that checks all hunter pos against the player's
    for (int i = 0; i < data.h_num * (spawn.count - 1); i++){
        if ((equal(hunter[i].loc, player.loc)) || ((equal(hunter[i].prev, player.loc)) && equal(hunter[i].loc, player.prev))){
            return true;
        }
    }
    return false;
...
void printmove(entity player,entity* hunters, entity* ships, spawnpoint spawn, round data, int sspawn){
    drawChar('P', player.loc.col, player.loc.row);//Draws the P
